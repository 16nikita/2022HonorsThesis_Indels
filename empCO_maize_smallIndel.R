library(dplyr)
#QUICK RELOAD:
PCO<-read.delim("penny_CO_v4.bed")
PCO_2kb_up<-read.delim("PCO_2kb_up.bed")
PCO_2kb_down<-read.delim("PCO_2kb_down.bed")
PCO_coldspots<-read.delim("PCO_random.bed")
PCO_cold_2kb_up<-read.delim("PCO_cold_2kb_up.bed")
PCO_cold_2kb_down<-read.delim("PCO_cold_2kb_down.bed")

#convert penny's data to v4 & create up/down
{
  #PREP COLDSPOT DATAFRAME FOR CROSSMAP: (convert v2 to v4)
  PCO<-read.delim("penny_CO_v2.bed",header=FALSE)
  colnames(PCO)<-c('chrom','start','end')
  PCO$midpoint<-as.integer((PCO$start+PCO$end)/2)
  PCO$length<-PCO$end-PCO$midpoint
  PCO$start<-PCO$midpoint
  PCO$end<-PCO$midpoint+1
  write.table(PCO,"PCO_v2.bed", row.names = F,quote=FALSE, sep = '\t')
  
  #READING CO COlDSPOTS CROSSMAP OUTPUT:
  PCO<-read.delim("PCO_v4.bed",sep='\t', header = FALSE)
  colnames(PCO)<-c("chrom","start","end","midpoint","length")
  PCO$end<-PCO$start+PCO$length
  PCO$start<-PCO$start-PCO$length
  PCO$length<-PCO$end-PCO$start
  PCO<-PCO[!grepl("B73", PCO$chrom),]
  PCO<-PCO[which(PCO$start>0),]
  write.table(PCO,"penny_CO_v4.bed", row.names = F,quote=FALSE, sep = '\t')
  
  CO<-read.delim("penny_CO_v4.bed",header=TRUE)
  ...
  CO_2kb_up <- CO_up(data.frame(matrix(ncol = 3,nrow = nrow(CO))),CO)
  CO_2kb_down <-CO_down(data.frame(matrix(ncol = 3,nrow = nrow(CO))),CO)
  write.table(CO_2kb_up,"PCO_2kb_up.bed", row.names = F,quote=FALSE, sep = '\t')
  write.table(CO_2kb_down,"PCO_2kb_down.bed", row.names = F,quote=FALSE, sep = '\t')
}
...
#FOR METAPLOTS: extending all 10kb
{
  ...
  write.table(hotspots_2[-c(4:5)],"penny_CO_v4_2.bed", row.names = F,quote=FALSE, sep = '\t')
  write.table(hotspots_2_2kbup,"PCO_2kb_up_2.bed", row.names = F,quote=FALSE, sep = '\t')
  write.table(hotspots_2_2kbdown,"PCO_2kb_down_2.bed", row.names = F,quote=FALSE, sep = '\t')
}
...
#Create dataframe of "coldspots" that do not overlap with one another, excluding hotspots, sample from list of CO desert regions
{
  ...
  #store row counts
  chrom1<- nrow(PCO[which(PCO$chrom == 1),])
  chrom2<- nrow(PCO[which(PCO$chrom == 2),])
  chrom3<- nrow(PCO[which(PCO$chrom == 3),])
  chrom4<- nrow(PCO[which(PCO$chrom == 4),])
  chrom5<- nrow(PCO[which(PCO$chrom == 5),])
  chrom6<- nrow(PCO[which(PCO$chrom == 6),])
  chrom7<- nrow(PCO[which(PCO$chrom == 7),])
  chrom8<- nrow(PCO[which(PCO$chrom == 8),])
  chrom9<- nrow(PCO[which(PCO$chrom == 9),])
  chrom10<- nrow(PCO[which(PCO$chrom == 10),])
  ...
  write.table(hotspot_random,"PCO_random_v2.bed", row.names = F,quote=FALSE, sep = '\t')
  
  #READING CO COlDSPOTS CROSSMAP OUTPUT:
  hotspot_random<-read.delim("PCO_random_v2.bed",sep='\t', header = TRUE)
  ...
  write.table(hotspot_random,"PCO_random.bed", row.names = F,quote=FALSE, sep = '\t')
  ...
  write.table(hotspot_random_2kb_up,"PCO_cold_2kb_up.bed", row.names = F,quote=FALSE, sep = '\t')
  write.table(hotspot_random_2kb_down,"PCO_cold_2kb_down.bed", row.names = F,quote=FALSE, sep = '\t')
}

##READ IN INTERSECT DATA:
# INTERSECT WITH PREDICTED CO REGIONS
{
  intersect_pred<-read.delim("intersect_a_PCO.txt",sep='\t', header = FALSE)
  ...
  intersect_10bp_pred<-read.delim("intersect_10bp_a_PCO.txt",sep='\t', header = FALSE)
  ...
  intersect_20bp_pred<-read.delim("intersect_20bp_a_PCO.txt",sep='\t', header = FALSE)
  ...
  intersect_30bp_pred<-read.delim("intersect_30bp_a_PCO.txt",sep='\t', header = FALSE)
  ...
  intersect_40bp_pred<-read.delim("intersect_40bp_a_PCO.txt",sep='\t', header = FALSE)
  ...
  intersect_50bp_pred<-read.delim("intersect_50bp_a_PCO.txt",sep='\t', header = FALSE)
  ...
  intersect_2kbup_pred<-read.delim("intersect_2kbup_a_PCO.txt",sep='\t', header = FALSE)
  ...
  intersect_10bp_2kbup_pred<-read.delim("intersect_10bp_2kbup_a_PCO.txt",sep='\t', header = FALSE)
  ...
  intersect_20bp_2kbup_pred<-read.delim("intersect_20bp_2kbup_a_PCO.txt",sep='\t', header = FALSE)
  ...
  intersect_30bp_2kbup_pred<-read.delim("intersect_30bp_2kbup_a_PCO.txt",sep='\t', header = FALSE)
  ...
  intersect_40bp_2kbup_pred<-read.delim("intersect_40bp_2kbup_a_PCO.txt",sep='\t', header = FALSE)
  ...
  intersect_50bp_2kbup_pred<-read.delim("intersect_50bp_2kbup_a_PCO.txt",sep='\t', header = FALSE)
  ...
  intersect_2kbdown_pred<-read.delim("intersect_2kbdown_a_PCO.txt",sep='\t', header = FALSE)
  ...
  intersect_10bp_2kbdown_pred<-read.delim("intersect_10bp_2kbdown_a_PCO.txt",sep='\t', header = FALSE)
  ...
  intersect_20bp_2kbdown_pred<-read.delim("intersect_20bp_2kbdown_a_PCO.txt",sep='\t', header = FALSE)
  ...
  intersect_30bp_2kbdown_pred<-read.delim("intersect_30bp_2kbdown_a_PCO.txt",sep='\t', header = FALSE)
  ...
  intersect_40bp_2kbdown_pred<-read.delim("intersect_40bp_2kbdown_a_PCO.txt",sep='\t', header = FALSE)
  ...
  intersect_50bp_2kbdown_pred<-read.delim("intersect_50bp_2kbdown_a_PCO.txt",sep='\t', header = FALSE)
...
{
  intersect_cold_pred<-read.delim("intersect_a_randPCO.txt",sep='\t', header = FALSE)[-c(7:8)]
  ...
  intersect_10bp_cold_pred<-read.delim("intersect_10bp_a_randPCO.txt",sep='\t', header = FALSE)[-c(7:8)]
  ...
  intersect_20bp_cold_pred<-read.delim("intersect_20bp_a_randPCO.txt",sep='\t', header = FALSE)[-c(7:8)]
  ...
  intersect_30bp_cold_pred<-read.delim("intersect_30bp_a_randPCO.txt",sep='\t', header = FALSE)[-c(7:8)]
  ...
  intersect_40bp_cold_pred<-read.delim("intersect_40bp_a_randPCO.txt",sep='\t', header = FALSE)[-c(7:8)]
  ...
  intersect_50bp_cold_pred<-read.delim("intersect_50bp_a_randPCO.txt",sep='\t', header = FALSE)[-c(7:8)]
  ...
  intersect_2kbup_cold_pred<-read.delim("intersect_2kbup_a_randPCO.txt",sep='\t', header = FALSE)
  ...
  intersect_10bp_2kbup_cold_pred<-read.delim("intersect_10bp_2kbup_a_randPCO.txt",sep='\t', header = FALSE)
  ...
  intersect_20bp_2kbup_cold_pred<-read.delim("intersect_20bp_2kbup_a_randPCO.txt",sep='\t', header = FALSE)
  ...
  intersect_30bp_2kbup_cold_pred<-read.delim("intersect_30bp_2kbup_a_randPCO.txt",sep='\t', header = FALSE)
  ...
  intersect_40bp_2kbup_cold_pred<-read.delim("intersect_40bp_2kbup_a_randPCO.txt",sep='\t', header = FALSE)
  ...
  intersect_50bp_2kbup_cold_pred<-read.delim("intersect_50bp_2kbup_a_randPCO.txt",sep='\t', header = FALSE)
  ...
  intersect_2kbdown_cold_pred<-read.delim("intersect_2kbdown_a_randPCO.txt",sep='\t', header = FALSE)
  ...
  intersect_10bp_2kbdown_cold_pred<-read.delim("intersect_10bp_2kbdown_a_randPCO.txt",sep='\t', header = FALSE)
  ...
  intersect_20bp_2kbdown_cold_pred<-read.delim("intersect_20bp_2kbdown_a_randPCO.txt",sep='\t', header = FALSE)
  ...
  intersect_30bp_2kbdown_cold_pred<-read.delim("intersect_30bp_2kbdown_a_randPCO.txt",sep='\t', header = FALSE)
  ...
  intersect_40bp_2kbdown_cold_pred<-read.delim("intersect_40bp_2kbdown_a_randPCO.txt",sep='\t', header = FALSE)
  ...
  intersect_50bp_2kbdown_cold_pred<-read.delim("intersect_50bp_2kbdown_a_randPCO.txt",sep='\t', header = FALSE)
...
indels_PCO<-intersect_pred[c(1:3)]
colnames(indels_PCO)<-c("chrom","start","end")
indels_coldPCO<-intersect_cold_pred[c(1:3)]
colnames(indels_coldPCO)<-c("chrom","start","end")
write.table(indels_PCO,file="indels_PCO.bed",row.names = F,quote=FALSE, sep = '\t')
write.table(indels_coldPCO,file="indels_coldPCO.bed",row.names = F,quote=FALSE, sep = '\t')


##STATISTIC 1: INDEL OVERLAP
#Calculate percentage of Indels that overlap with PCOs
...
write.table(stat1,file="stat1_1_indel_PCO.txt",row.names = T,quote=FALSE, sep = '\t')
...
write.table(stat1_2,file="stat1_2_indel_PCO.txt",row.names = T,quote=FALSE, sep = '\t')
...
write.table(stat1_3,file="stat1_3_indel_PCO.txt",row.names = T,quote=FALSE, sep = '\t')
...
write.table(stat1_4,file="stat1_4_indel_PCO.txt",row.names = T,quote=FALSE, sep = '\t')
}
...
{
intersect_overlap<-percentage_overlap(intersect_pred,PCO)
intersect_10bp_overlap<-percentage_overlap(intersect_10bp_pred,PCO)
intersect_20bp_overlap<-percentage_overlap(intersect_20bp_pred,PCO)
intersect_30bp_overlap<-percentage_overlap(intersect_30bp_pred,PCO)
intersect_40bp_overlap<-percentage_overlap(intersect_40bp_pred,PCO)
intersect_50bp_overlap<-percentage_overlap(intersect_50bp_pred,PCO)

intersect_2kbup_overlap<-percentage_overlap(intersect_2kbup_pred,PCO)
intersect_10bp_2kbup_overlap<-percentage_overlap(intersect_10bp_2kbup_pred,PCO)
intersect_20bp_2kbup_overlap<-percentage_overlap(intersect_20bp_2kbup_pred,PCO)
intersect_30bp_2kbup_overlap<-percentage_overlap(intersect_30bp_2kbup_pred,PCO)
intersect_40bp_2kbup_overlap<-percentage_overlap(intersect_40bp_2kbup_pred,PCO)
intersect_50bp_2kbup_overlap<-percentage_overlap(intersect_50bp_2kbup_pred,PCO)

intersect_2kbdown_overlap<-percentage_overlap(intersect_2kbdown_pred,PCO)
intersect_10bp_2kbdown_overlap<-percentage_overlap(intersect_10bp_2kbdown_pred,PCO)
intersect_20bp_2kbdown_overlap<-percentage_overlap(intersect_20bp_2kbdown_pred,PCO)
intersect_30bp_2kbdown_overlap<-percentage_overlap(intersect_30bp_2kbdown_pred,PCO)
intersect_40bp_2kbdown_overlap<-percentage_overlap(intersect_40bp_2kbdown_pred,PCO)
intersect_50bp_2kbdown_overlap<-percentage_overlap(intersect_50bp_2kbdown_pred,PCO)

intersect_cold_overlap<-percentage_overlap(intersect_cold_pred,PCO_coldspots)
intersect_10bp_cold_overlap<-percentage_overlap(intersect_10bp_cold_pred,PCO_coldspots)
intersect_20bp_cold_overlap<-percentage_overlap(intersect_20bp_cold_pred,PCO_coldspots)
intersect_30bp_cold_overlap<-percentage_overlap(intersect_30bp_cold_pred,PCO_coldspots)
intersect_40bp_cold_overlap<-percentage_overlap(intersect_40bp_cold_pred,PCO_coldspots)
intersect_50bp_cold_overlap<-percentage_overlap(intersect_50bp_cold_pred,PCO_coldspots)

intersect_2kbup_cold_overlap<-percentage_overlap(intersect_2kbup_cold_pred,PCO_cold_2kb_up)
intersect_10bp_2kbup_cold_overlap<-percentage_overlap(intersect_10bp_2kbup_cold_pred,PCO_cold_2kb_up)
intersect_20bp_2kbup_cold_overlap<-percentage_overlap(intersect_20bp_2kbup_cold_pred,PCO_cold_2kb_up)
intersect_30bp_2kbup_cold_overlap<-percentage_overlap(intersect_30bp_2kbup_cold_pred,PCO_cold_2kb_up)
intersect_40bp_2kbup_cold_overlap<-percentage_overlap(intersect_40bp_2kbup_cold_pred,PCO_cold_2kb_up)
intersect_50bp_2kbup_cold_overlap<-percentage_overlap(intersect_50bp_2kbup_cold_pred,PCO_cold_2kb_up)

intersect_2kbdown_cold_overlap<-percentage_overlap(intersect_2kbdown_cold_pred,PCO_cold_2kb_down)
intersect_10bp_2kbdown_cold_overlap<-percentage_overlap(intersect_10bp_2kbdown_cold_pred,PCO_cold_2kb_down)
intersect_20bp_2kbdown_cold_overlap<-percentage_overlap(intersect_20bp_2kbdown_cold_pred,PCO_cold_2kb_down)
intersect_30bp_2kbdown_cold_overlap<-percentage_overlap(intersect_30bp_2kbdown_cold_pred,PCO_cold_2kb_down)
intersect_40bp_2kbdown_cold_overlap<-percentage_overlap(intersect_40bp_2kbdown_cold_pred,PCO_cold_2kb_down)
intersect_50bp_2kbdown_cold_overlap<-percentage_overlap(intersect_50bp_2kbdown_cold_pred,PCO_cold_2kb_down)
}
...
#T-TEST:
{
  ...
  write.table(stat1,file="stat3_1_indel_PCO.txt",row.names = T,quote=FALSE, sep = '\t')
  
  stat1_2<-as.data.frame(cbind( intersect_10bp_overlap,intersect_10bp_cold_overlap,intersect_20bp_overlap,intersect_20bp_cold_overlap,intersect_30bp_overlap,intersect_30bp_cold_overlap,intersect_40bp_overlap, intersect_40bp_cold_overlap,intersect_50bp_overlap,intersect_50bp_cold_overlap))
  stat1_2[nrow(stat1_2) + 1,] <- colMeans(as.matrix(stat1_2))
  stat1_2[nrow(stat1_2) + 1,] <- colVars(as.matrix(stat1_2))
  stat1_2[is.na(stat1_2)]<-0
  stat1_2[nrow(stat1_2) + 1,] <-NA
  rownames(stat1_2)<-c("chr1","chr2","chr3","chr4","chr5","chr6","chr7","chr8","chr9","chr10","mean","variance","p-value")
  options(scipen=999)
  for(i in seq(from=1, to=ncol(stat1_2)-1, by=2)){
  ...
  write.table(stat1_2,file="stat3_2_indel_PCO.txt",row.names = T,quote=FALSE, sep = '\t')
  ...
  write.table(stat1_3,file="stat3_3_indel_PCO.txt",row.names = T,quote=FALSE, sep = '\t')
  ...
  write.table(stat1_4,file="stat3_4_indel_PCO.txt",row.names = T,quote=FALSE, sep = '\t')
}
...
test<-cbind(test,"Mean Indel / PCO Overlap"=c(mean(data[which(data$type == "all deletions"),]$value),mean(data[which(data$type == "1-10bp"),]$value),mean(data[which(data$type == "11-20bp"),]$value),mean(data[which(data$type == "21-30bp"),]$value),mean(data[which(data$type == "31-40bp"),]$value),mean(data[which(data$type == "41-50bp"),]$value)))
...
test<-cbind(test,"Mean PCO Overlap"=c(mean(data[which(data$type == "all deletions"),]$value),mean(data[which(data$type == "1-10bp"),]$value),mean(data[which(data$type == "11-20bp"),]$value),mean(data[which(data$type == "21-30bp"),]$value),mean(data[which(data$type == "31-40bp"),]$value),mean(data[which(data$type == "41-50bp"),]$value)))
...
test<-cbind(test,"Mean Indel Density on PCO (indels/bp)"=c(mean(data[which(data$type == "all deletions"),]$value),mean(data[which(data$type == "1-10bp"),]$value),mean(data[which(data$type == "11-20bp"),]$value),mean(data[which(data$type == "21-30bp"),]$value),mean(data[which(data$type == "31-40bp"),]$value),mean(data[which(data$type == "41-50bp"),]$value)))
...
write.table(test_2,file="PCOs_a_output.txt",row.names = T,quote=FALSE, sep = '\t')
...
#BARGRAPHS - for each: change TITLE, INDEL SIZES, LEGEND, AXIS TITLE POSITION
{
  ...
  perc<-ggplot(full_data1, aes(fill=color,x=levels,y=value)) +
    ...
    annotate("text", c(1.5,3.5,5.5,7.5,9.5), y = -.001, label = c("1-10 bp","11-20 bp","21-30 bp","31-40 bp","41-50 bp")) +
  ...
  perc2<-ggplot(full_data2, aes(fill=color,x=levels,y=value)) +
    ...
    annotate("text", x = 1:2, y = -.0005,
             label = rep(c("Negative Control Region", "Empirical CO Region"), 1)) +
...
}
{
  metaplot<-function(all, all_cold){
    ...
    test2<-unlist(test2)
    interval <-plot(smooth.spline(seq(-1000,999,100),test,spar=0.6),col="red",lwd=1,type="l",ylim=c(0,max),main="Indel Densities at PCOs",xlab ="Distance from midpoint", ylab="Indel Density (indels/bp)")+lines(smooth.spline(seq(-1000,999,100),test2,spar=0.6),col="blue",lwd=1,type="l",ylim=c(),main="Indel Densities at PCOs",xlab ="Distance from midpoint", ylab="Indel Density (indels/bp)")
    ...
    test2<-unlist(test2)
    Twokbdown <-plot(smooth.spline(seq(-1000,999,100),test,spar=0.6),col="red",lwd=1,type="l",ylim=c(0,max),main="Indel Densities 2kb down from PCOs",xlab ="Distance from midpoint", ylab="Indel Density (indels/bp)")+lines(smooth.spline(seq(-1000,999,100),test2,spar=0.6),col="blue",lwd=1,type="l",ylim=c(),main="Indel Densities 2kb down from PCOs",xlab ="Distance from midpoint", ylab="Indel Density (indels/bp)")
    ...
    test2<-unlist(test2)
    Twokbup <- plot(smooth.spline(seq(-1000,999,100),test,spar=0.6),col="red",lwd=1,type="l",ylim=c(0,max),main="Indel Densities 2kb up from PCOs",xlab ="Distance from midpoint", ylab="Indel Density (indels/bp)")+lines(smooth.spline(seq(-1000,999,100),test2,spar=0.6),col="blue",lwd=1,type="l",ylim=c(),main="Indel Densities 2kb up from PCOs",xlab ="Distance from midpoint", ylab="Indel Density (indels/bp)")
