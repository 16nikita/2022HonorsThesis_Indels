library(dplyr)
#QUICK RELOAD:
DSB<-read.delim("diplo_diplo_down_MLH3.narrowPeak",header=FALSE)[c(1:3)]
colnames(DSB)<-c('chrom','start','end')
write.table(DSB,file="mlh3_peaks.bed",row.names = F,quote=FALSE, sep = '\t')
...
{
  intersect_pred<-read.delim("intersect_a_mlh3.txt",sep='\t', header = FALSE)
  ...
  intersect_DSB<-read.delim("intersect_a_DSB.txt",sep='\t', header = FALSE)
  intersect_DSB$type<-"all deletions"
  intersect_DSB$intersect<-"intersect"
  intersect_DSB<-intersect_DSB[-c(7:8)]
  colnames(intersect_DSB) <- c('A_chrom', 'A_start', 'A_end','B_chrom', 'B_start', 'B_end','overlap in bp','type','intersection')
  intersect_hotspot<-read.delim("intersect_a_hotspot.txt",sep='\t', header = FALSE)
  intersect_hotspot$type<-"all indels"
  intersect_hotspot$intersect<-"intersect"
  colnames(intersect_hotspot) <- c('A_chrom', 'A_start', 'A_end','B_chrom', 'B_start', 'B_end','overlap in bp','type','intersection')
  intersect_empCO<-read.delim("intersect_a_PCO.txt",sep='\t', header = FALSE)
  intersect_empCO$type<-"all deletions"
  intersect_empCO$intersect<-"intersect"
  intersect_empCO<-intersect_empCO[-c(7:8)]
  colnames(intersect_empCO) <- c('A_chrom', 'A_start', 'A_end','B_chrom', 'B_start', 'B_end','overlap in bp','type','intersection')
  intersect_predCO<-read.delim("intersect_a_pred.txt",sep='\t', header = FALSE)
  intersect_predCO$type<-"all deletions"
  intersect_predCO$intersect<-"intersect"
  colnames(intersect_predCO) <- c('A_chrom', 'A_start', 'A_end','B_chrom', 'B_start', 'B_end','overlap in bp','type','intersection')
  
  library(dplyr)
  
  intersect_DSB_2<- (intersect_DSB %>% distinct(B_start, .keep_all = TRUE) )[c(4:6)]
  colnames(intersect_DSB_2)<-c('chrom','start','end')
  intersect_hotspot_2<- (intersect_hotspot %>% distinct(B_start, .keep_all = TRUE))[c(4:6)]
  colnames(intersect_hotspot_2)<-c('chrom','start','end')
  intersect_empCO_2<- (intersect_empCO %>% distinct(B_start, .keep_all = TRUE))[c(4:6)]
  colnames(intersect_empCO_2)<-c('chrom','start','end')
  intersect_predCO_2<- (intersect_predCO %>% distinct(B_start, .keep_all = TRUE))[c(4:6)]
  colnames(intersect_predCO_2)<-c('chrom','start','end')
  
  write.table(intersect_DSB_2,file="intersected_DSBs.bed",row.names = F,quote=FALSE, sep = '\t')
  write.table(intersect_hotspot_2,file="intersected_hotspots.bed",row.names = F,quote=FALSE, sep = '\t')
  write.table(intersect_empCO_2,file="intersected_empCOs.bed",row.names = F,quote=FALSE, sep = '\t')
  write.table(intersect_predCO_2,file="intersected_predCOs.bed",row.names = F,quote=FALSE, sep = '\t')
  }
...
}
{
  ...
  intersect_DSB_overlap<-percentage_overlap(intersect_DSB)
  intersect_predCO_overlap<-percentage_overlap(intersect_predCO)
  intersect_hotspot_overlap<-percentage_overlap(intersect_hotspot)
  intersect_empCO_overlap<-percentage_overlap(intersect_empCO)
...
#ANOVA test for significance
{
  ...
  {
    intersect_overlap<-percentage_overlap(intersect_pred,"MLH3 Binding Site")
    intersect_DSB_overlap<-percentage_overlap(intersect_DSB,"DSB Hotspot")
    intersect_predCO_overlap<-percentage_overlap(intersect_predCO,"Predicted CO")
    intersect_hotspot_overlap<-percentage_overlap(intersect_hotspot,"CO Hotspot")
    intersect_empCO_overlap<-percentage_overlap(intersect_empCO,"Empirical CO")
   }
  
  #STATISTIC 1
  stat2<-as.data.frame(rbind(intersect_overlap,intersect_DSB_overlap,intersect_predCO_overlap,intersect_hotspot_overlap,intersect_empCO_overlap))
  data1 <- data.frame(y=(stat2$percent),group=factor(stat2$dataset))
  ...
  quartiles_DSB<- quantile(data1[which(data1$group=='DSB'),]$y, probs=c(.25, .75), na.rm = FALSE)
  IQR_DSB <- IQR(data1[which(data1$group=='DSB'),]$y)
  Lower_DSB <- quartiles_DSB[1] - 1.5*IQR_DSB
  Upper_DSB <- quartiles_DSB[2] + 1.5*IQR_DSB 
  DSB_no_outlier <- subset(data1[which(data1$group=='DSB'),], data1$y > Lower & data1$y < Upper)
  
  DSB_sub<-data1[which(data1$group=='DSB'),]
  quartiles_DSB<- quantile(DSB_sub$y, probs=c(.25, .75), na.rm = FALSE)
  IQR_DSB <- IQR(DSB_sub$y)
  Lower_DSB <- quartiles_DSB[1] - 1.5*IQR_DSB
  Upper_DSB <- quartiles_DSB[2] + 1.5*IQR_DSB 
  DSB_no_outlier <- subset(DSB_sub, DSB_sub$y > Lower & DSB_sub$y < Upper)
  
  
  ggplot(data1, aes(x=factor(group), y=y, fill=group)) +
    geom_boxplot()+theme_classic() +xlab("Dataset")+ylab("Indel Density (indels/bp)")
...
#BARGRAPHS - for each: change TITLE, INDEL SIZES, LEGEND, AXIS TITLE POSITION
{
  ...
  perc<-ggplot(full_data1, aes(fill=color,x=levels,y=value)) +
    ...
    ggtitle("Small Indel Density on DSBs") +
...
}
{
  metaplot<-function(all, all_cold){
    ...
    test2<-unlist(test2)
    interval <-plot(smooth.spline(seq(-1000,999,100),test,spar=0.6),col="red",lwd=1,type="l",ylim=c(0,max),main="Indel Densities at DSBs",xlab ="Distance from midpoint", ylab="Indel Density (indels/bp)")+lines(smooth.spline(seq(-1000,999,100),test2,spar=0.6),col="blue",lwd=1,type="l",ylim=c(),main="Indel Densities at DSBs",xlab ="Distance from midpoint", ylab="Indel Density (indels/bp)")
    ...
    test2<-unlist(test2)
    Twokbdown <-plot(smooth.spline(seq(-1000,999,100),test,spar=0.6),col="red",lwd=1,type="l",ylim=c(0,max),main="Indel Densities 2kb down from DSBs",xlab ="Distance from midpoint", ylab="Indel Density (indels/bp)")+lines(smooth.spline(seq(-1000,999,100),test2,spar=0.6),col="blue",lwd=1,type="l",ylim=c(),main="Indel Densities 2kb down from DSBs",xlab ="Distance from midpoint", ylab="Indel Density (indels/bp)")
    ...
    test2<-unlist(test2)
    Twokbup <- plot(smooth.spline(seq(-1000,999,100),test,spar=0.6),col="red",lwd=1,type="l",ylim=c(0,max),main="Indel Densities 2kb up from DSBs",xlab ="Distance from midpoint", ylab="Indel Density (indels/bp)")+lines(smooth.spline(seq(-1000,999,100),test2,spar=0.6),col="blue",lwd=1,type="l",ylim=c(),main="Indel Densities 2kb up from DSBs",xlab ="Distance from midpoint", ylab="Indel Density (indels/bp)")
