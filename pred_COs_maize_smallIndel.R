...
#MERGED COS (merge bookend/overlapping):
...
#CREATE UP/DOWN FROM MERGED RCO FILE:
{
  CO<-read.delim("CO_predicted_3.bed",header=FALSE)
  ...
  write.table(CO_2kb_up,"CO_pred_cold_2kb_up_2.bed", row.names = F,quote=FALSE, sep = '\t')
  write.table(CO_2kb_down,"CO_pred_cold_2kb_down_2.bed", row.names = F,quote=FALSE, sep = '\t')
  
}

#CREATE COLD REGIONS FOR MERGED RCO FILE:
{##Generate random data for NAM COs- negative control, unique regions that do not overlap with hotspots
  set.seed(200)
  ...
  CO_coldspots<-CO_coldspots[order(CO_coldspots$chrom,CO_coldspots$start),]
  library(dplyr)
  library(reshape)
  set.seed(200)
  ...
  #store row counts
  chrom1<- nrow(CO_predicted[which(CO_predicted$chrom == 1),])
  chrom2<- nrow(CO_predicted[which(CO_predicted$chrom == 2),])
  chrom3<- nrow(CO_predicted[which(CO_predicted$chrom == 3),])
  chrom4<- nrow(CO_predicted[which(CO_predicted$chrom == 4),])
  chrom5<- nrow(CO_predicted[which(CO_predicted$chrom == 5),])
  chrom6<- nrow(CO_predicted[which(CO_predicted$chrom == 6),])
  chrom7<- nrow(CO_predicted[which(CO_predicted$chrom == 7),])
  chrom8<- nrow(CO_predicted[which(CO_predicted$chrom == 8),])
  chrom9<- nrow(CO_predicted[which(CO_predicted$chrom == 9),])
  chrom10<- nrow(CO_predicted[which(CO_predicted$chrom == 10),])
  
  CO_coldspots_1<-create_coldspot(chrom1,CO_coldspots[which(CO_coldspots$chrom == 1),],1,3752)
  CO_coldspots_2<-create_coldspot(chrom2,CO_coldspots[which(CO_coldspots$chrom == 2),],2,3752)
  CO_coldspots_3<-create_coldspot(chrom3,CO_coldspots[which(CO_coldspots$chrom == 3),],3,3752)
  CO_coldspots_4<-create_coldspot(chrom4,CO_coldspots[which(CO_coldspots$chrom == 4),],4,3752)
  CO_coldspots_5<-create_coldspot(chrom5,CO_coldspots[which(CO_coldspots$chrom == 5),],5,3752)
  CO_coldspots_6<-create_coldspot(chrom6,CO_coldspots[which(CO_coldspots$chrom == 6),],6,3752)
  CO_coldspots_7<-create_coldspot(chrom7,CO_coldspots[which(CO_coldspots$chrom == 7),],7,3752)
  CO_coldspots_8<-create_coldspot(chrom8,CO_coldspots[which(CO_coldspots$chrom == 8),],8,3752)
  CO_coldspots_9<-create_coldspot(chrom9,CO_coldspots[which(CO_coldspots$chrom == 9),],9,3752)
  CO_coldspots_10<-create_coldspot(chrom10,CO_coldspots[which(CO_coldspots$chrom == 10),],10,3752)
  
  #Bind to create dataframe with all random regions
  PredCO_random<-rbind(CO_coldspots_1,CO_coldspots_2,CO_coldspots_3,CO_coldspots_4,CO_coldspots_5,CO_coldspots_6,CO_coldspots_7,CO_coldspots_8,CO_coldspots_9,CO_coldspots_10)
  
  #PREP COLDSPOT DATAFRAME FOR CROSSMAP: (convert v2 to v4)
  PredCO_random$midpoint<-as.integer((PredCO_random$start+PredCO_random$end)/2)
  PredCO_random$length<-PredCO_random$end-PredCO_random$midpoint
  PredCO_random$start<-PredCO_random$midpoint
  PredCO_random$end<-PredCO_random$midpoint+1
  write.table(PredCO_random,"CO_random_v2.bed", row.names = F,quote=FALSE, sep = '\t')
  
  #READING CO COlDSPOTS CROSSMAP OUTPUT:
  PredCO_random<-read.delim("CO_random_v4.bed",sep='\t', header = FALSE)
  colnames(PredCO_random)<-c("chrom","start","end","midpoint","length")
  PredCO_random$end<-PredCO_random$start+PredCO_random$length
  PredCO_random$start<-PredCO_random$start-PredCO_random$length
  PredCO_random$length<-PredCO_random$end-PredCO_random$start
  PredCO_random<-PredCO_random[!grepl("B73", PredCO_random$chrom),]
  PredCO_random<-PredCO_random[which(PredCO_random$start>0),]
  write.table(PredCO_random,"CO_pred_coldspots_2.bed", row.names = F,quote=FALSE, sep = '\t')
  
  #Creating datasets of 2kb upstream and 2kb downstream from CO "coldspot" intervals
  PredCO_random_2kb_up <- CO_up(data.frame(matrix(ncol = 3,nrow = nrow(PredCO_random))),PredCO_random)
  PredCO_random_2kb_down <-CO_down(data.frame(matrix(ncol = 3,nrow = nrow(PredCO_random))),PredCO_random)
  write.table(PredCO_random_2kb_up,"CO_pred_2kb_up_2.bed", row.names = F,quote=FALSE, sep = '\t')
  write.table(PredCO_random_2kb_down,"CO_pred_2kb_down_2.bed", row.names = F,quote=FALSE, sep = '\t')
}
...
indels_predCO<-intersect_pred[c(1:3)]
colnames(indels_predCO)<-c("chrom","start","end")
indels_coldpredCO<-intersect_cold_pred[c(1:3)]
colnames(indels_coldpredCO)<-c("chrom","start","end")
write.table(indels_predCO,file="indels_predCO.bed",row.names = F,quote=FALSE, sep = '\t')
write.table(indels_coldpredCO,file="indels_coldpredCO.bed",row.names = F,quote=FALSE, sep = '\t')
...
#BARGRAPHS - for each: change TITLE, INDEL SIZES, LEGEND, AXIS TITLE POSITION
{
  ...
  perc<-ggplot(full_data1, aes(fill=color,x=levels,y=value)) +
    ...
    ggtitle("Small Indel Density on Predicted COs") +
    ...
    annotate("text", c(1.5,3.5,5.5,7.5,9.5), y = -.0008, label = c("1-10 bp","11-20 bp","21-30 bp","31-40 bp","41-50 bp")) +
